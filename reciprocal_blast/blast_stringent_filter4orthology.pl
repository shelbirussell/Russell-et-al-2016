...
while (<BLAST>) {
    ...
    if (! exists $hits{$qseqid}) {
        ...
        # if more than 1 hit, only keep hits that have evalues > 3 orders of magnitude better than any other hits (unambiguous best)
        if (($evalue+1e-308)/($hits{$qseqid}{"Evalue"}+1e-308) >= 1000) {
            $hits{$qseqid}{"SUBJECT"} = $sallacc ;
            $hits{$qseqid}{"TITLES"} = $salltitles ;
            $hits{$qseqid}{"QCOV"} = $query_coverage ;
            $hits{$qseqid}{"PIDENTITY"} = $pident ;
